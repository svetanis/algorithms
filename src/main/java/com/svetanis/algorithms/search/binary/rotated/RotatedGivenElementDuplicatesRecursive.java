...
public final class RotatedGivenElementDuplicatesRecursive {
	...
	public boolean searchRotated(int[] a, int k) {
		int n = a.length;
		return searchRotated(a, 0, n - 1, k);
	}

	private boolean searchRotated(int[] a, int left, int right, int k) {
		if (right < left) {
			return false;
		}
		int mid = left + (right - left) / 2;
		...
		if (a[left] < a[mid]) {
			// left is sorted
			if (k >= a[left] && k < a[mid]) {
				return searchRotated(a, left, mid - 1, k);
			} else {
				return searchRotated(a, mid + 1, right, k);
			}
		} else if (a[mid] < a[left]) {
			// right is sorted
			if (k >= a[mid] && k <= a[right]) {
		...
		// (with the right half being all dups)
			...
			if (a[mid] != a[right]) {
				...
				// we have to search both halves
				boolean result = searchRotated(a, left, mid - 1, k);
				if (!result) {
					return searchRotated(a, mid + 1, right, k);
				} else {
					return result;
