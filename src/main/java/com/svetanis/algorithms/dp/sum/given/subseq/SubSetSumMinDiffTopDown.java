...
public final class SubSetSumMinDiffTopDown {
  ...
  public static int minDiff(int[] a) {
    ...
    return minDiff(a, 0, 0, 0, dp);
  }

  private static int minDiff(int[] a, int i, int sum1, int sum2, Integer[][] dp) {
    int n = a.length;
    ...
    if (dp[i][sum1] == null) {
      ...
      int incl = minDiff(a, i + 1, sum1 + a[i], sum2, dp);

      // 2. exclude element from first subset
      int excl = minDiff(a, i + 1, sum1, sum2 + a[i], dp);

      dp[i][sum1] = min(incl, excl);
    }
    return dp[i][sum1];
  }
