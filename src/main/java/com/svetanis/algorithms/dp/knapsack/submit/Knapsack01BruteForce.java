...
public final class Knapsack01BruteForce {
	...
	public static int knapsack(List<Integer> weights, List<Integer> values, int max) {
		...
		return knapsack(weights, values, max, n - 1);
	}

	private static int knapsack(List<Integer> w, List<Integer> v, int max, int n) {
		...
		// the available weight, skip the current item
		if (w.get(n) > max) {
		...
		int incl = v.get(n) + knapsack(w, v, max - w.get(n), n - 1);
