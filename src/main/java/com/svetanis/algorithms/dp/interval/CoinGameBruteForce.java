...
public final class CoinGameBruteForce {
	// Time Complexity: O(n * 2^n)

	public static int game(List<Integer> coins) {
		int n = coins.size();
		return maxScore(coins, 0, n - 1);
	}

	private static int maxScore(List<Integer> coins, int left, int right) {
		if (left == right) {
			return coins.get(right);
		}
		int sum = 0;
		for (int i = left; i <= right; i++) {
			sum += coins.get(i);
		}
		int leftPick = maxScore(coins, left + 1, right);
		int rightPick = maxScore(coins, left, right - 1);
